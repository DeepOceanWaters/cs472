...
\begin{description}
    ...
    A misaligned operand is offset in an unexpected fashion. It's a problem in programming because you have to check for the misalignment, and deal with it.
    ...
    Because ARM processors are pipelined, the next instruction (based on the contents of r15) is fetched before the current instruction is finished running. This means that, if the current instruction was meant to update r15 to point to a new instruction, the wrong instruction would be fetched.
